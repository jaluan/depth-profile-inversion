function bedrockMCvJ4_joined(snr,fnr,rnr)
...
% forward_bedrock_vJ2_1.m (JLA)
% vJ3: Aug 2021, better documentation, and stone-calibrated production rates from
% compile_data_vJ3 (JLA)
% vJ4: Nov 2022, three exponentials to muon fit to depth of app. 20 m (JLA)
% _joined: optimized for multiple depth profiles of different lengths (JLA)

% function bedrockMCvJ4(snr,fnr,rnr)
...
if length(snr)==1
    if fnr==21
        load('./data/Lamuvaara_v4.mat','sample'), dpname = 'Lamuvaara_v4'; % 'sample', 'model'
    elseif fnr==22
        load('./data/Naakakarhakka_v4.mat','sample'), dpname = 'Naakakarhakka_v4'; % 'sample', 'model'
    else
        load(['./data/synthetic_data_fw_dp_v4_scn',num2str(fnr)],'sample','model'), 
        dpname = ['Syn_v4_scn',num2str(fnr),'_',num2str(rnr)']; % 'sample', 'model'
    end
elseif length(snr)==2
    load('./data/Naa_Lam_v4_n.mat','sample'), dpname = 'Naa_Lam_v4_full_n'; % 'sample', 'model' 
    fnr=30; %Used to get in right loop below, not pretty.
end
...
model.mp{1}.vmin = 3.5; %3.1
model.mp{1}.vmax = 5.0; %5.1
...
for i=1:model.Nsnr
    ...
    model.Ndp{i} = length(sample{i}.depths);

    %number of nuclides
    model.Nnc = sample{i}.Nnuc; %for now only handling samples with same nuclides measured

    %total number of data per sample (nuclides*depths)
    model.Nds{i} = model.Nnc*model.Ndp{i};

end

%save some general MCMC paramers
model.Nwalk = 10; %6; Number of walkers
model.burnin = 10e3; %6e3; Length of burnin
model.Nmod = 100e3; %60e3; Number of models
...
%data and covariance
lobs=sum([model.Nds{:}]); %length of observation vectors
dobs=zeros(1,lobs); sigd=zeros(1,lobs); %initiate data observation and error vectors
if fnr > 20 %Real data
    for i=1:model.Nsnr %loop samples
        for j=1:model.Ndp{i} %loop depthpoints
            if i==1
                dobs((i-1)*model.Nds{i}+j) = model.data{i}.N10(j);
                dobs((i-1)*model.Nds{i}+j+model.Ndp{i}) = model.data{i}.N26(j);
                sigd((i-1)*model.Nds{i}+j) = model.data{i}.dN10(j);
                sigd((i-1)*model.Nds{i}+j+model.Ndp{i}) = model.data{i}.dN26(j);
            elseif i==2
                dobs((i-1)*model.Nds{i-1}+j) = model.data{i}.N10(j);
                dobs((i-1)*model.Nds{i-1}+j+model.Ndp{i}) = model.data{i}.N26(j);
                sigd((i-1)*model.Nds{i-1}+j) = model.data{i}.dN10(j);
                sigd((i-1)*model.Nds{i-1}+j+model.Ndp{i}) = model.data{i}.dN26(j);
            else
                warning('Max of two sites can be modelled together at present')
            end
            if model.Nnc == 3 %needs updating if 21Ne is included
               dobs((i-1)*model.Nds{i}+j+2*model.Ndp{i}) = model.data{i}.N21(j);
               sigd((i-1)*model.Nds{i}+j+2*model.Ndp{i}) = model.data{i}.dN21(j);
            end
        end
    end
else %synthetic data
    for i=1:model.Nsnr
        for j=1:model.Ndp
            dobs((i-1)*model.Nds+j) = model.data{i}.N10(rnr,j);
            dobs((i-1)*model.Nds+j+model.Ndp) = model.data{i}.N26(rnr,j);
            sigd((i-1)*model.Nds+j) = model.data{i}.dN10(rnr,j);
            sigd((i-1)*model.Nds+j+model.Ndp) = model.data{i}.dN26(rnr,j);
            if model.Nnc == 3
               dobs((i-1)*model.Nds+j+2*model.Ndp) = model.data{i}.N21(rnr,j);
               sigd((i-1)*model.Nds+j+2*model.Ndp) = model.data{i}.dN21(rnr,j);
            end
        end
    end
end
    ...
    while ((mi < model.Nmax)&&(acount < model.Nmod))
       ...
       %acceptance ratio
        ...
        if model.Nnc(1) == 2 %10Be, 26Al 
            [gmp] = forward_bedrockvJ4_joined(up,model,CNprop); % _joined: handling depth profiles of different lengths
        elseif model.Nnc(1) == 3 %21Ne
            [gmp] = forward_bedrockvJ3_Ne(up,model,CNprop);  %needs update
        end
