function [gm] = forward_bedrockvJ4_joined(up,model,CNprop)

% function [gm] = forward_bedrockvJ4(up,model,CNprop)
% Forward model for transient CN integration of bedrock samples 
% _joined: optimized for multiple depth profiles of different lengths
...
%     burial = interp1(mT,mz,time); %Depths at times in 'time' %old
    ...
    Ndp=model.Ndp{i}; %Number of data points in depth profile
   ...
   % Pack output
    for j=1:Ndp %loop depthpoints
        if i==1
            gm((i-1)*model.Nds{i}+j) = N10(j,1);
            gm((i-1)*model.Nds{i}+Ndp+j) = N26(j,1);
        elseif i==2
            gm((i-1)*model.Nds{i-1}+j) = N10(j,1);
            gm((i-1)*model.Nds{i-1}+Ndp+j) = N26(j,1);
        end
    end
end
